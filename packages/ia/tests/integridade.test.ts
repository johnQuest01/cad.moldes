...
describe('Quem tem licença, e é pouca gente', () => {
  it('só três ferramentas têm licença, e a de mexer no desenho é UMA', () => {
    ...
    expect(mexemNaForma.map((f) => f.nome)).toEqual(['simplificar_contorno']);
    expect(comLicenca).toHaveLength(3);
  });
